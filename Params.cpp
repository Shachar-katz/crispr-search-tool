...
{
  for (map<string, vector<const char*> >::iterator it = m_values.begin(); it != m_values.end(); ++it) {
    string id = (*it).first;
    if (m_parsers.find(id) == m_parsers.end() && ignore_missing)
      continue;
    Parser* parser = get_parser(id);
    const char* value = get_value(id);
    parser->parse(value);
    m_set[id] = true;
  }
}
...
const char* Parameters::get_value(string id)
{
  if (m_values.find(id) == m_values.end()) {
    cerr << "value for option " << id << " not set" << endl;
    exit(-1);
  }
  vector<const char*>& values = m_values[id];
  if (values.size() != 1) {
